...
namespace HugoLand.Métier
{
    ...
    public static class Monde
    {
        ...
        public static void SupprimerMonde(Accès_aux_données.Monde id)
        {
            using (Accès_aux_données.Entities context = new Accès_aux_données.Entities())
            {
                Accès_aux_données.Monde monde = context.Mondes.Where(m => m == id).First();
                context.Mondes.Remove(monde);
                context.SaveChanges();
            }
        }

        public static void ChangerLimites(int limiteX, int limiteY, Accès_aux_données.Monde id)
        {
            using (Accès_aux_données.Entities context = new Accès_aux_données.Entities())
            {
        ...
        public static void ChangerDescription(string description, Accès_aux_données.Monde id)
        {
            using (Accès_aux_données.Entities context = new Accès_aux_données.Entities())
            {
                Accès_aux_données.Monde monde = context.Mondes.Where(m => m == id).First();
